...
def runtest(tools, alpha_val, budget, plen, hlen, seed):
    ...
    print ('For {} project, {} honeypots, where alpha = {}, the exp loss = {}'.format(plen,hlen,alpha_val, result))
...
for n in range(1,N):
    ...
    for pk in [5,8,12,15]:
    #for p1 in range(100,101,5):
        for p2 in range(15,16,5):
            for p3 in [-.5,0,.5]:
                ...
                try:
                    ...
                    result = runtest(pk, p3, 1000, 50, p2,n)
                    #end_time = time.time()
